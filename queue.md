...
- Status: proposed — awaiting review
- Claim: Gated Semantic MAP-Elites (GSME) keeps a quality-diversity archive of harness edits keyed on the (where × why) pathology an edit addresses — not the tasks it was tuned on — with one gated elite per cell and cross-cell recombination as an anti-overfitting mechanism; across 7 domains from terminal-bench-2, EvoAgentBench, and AppWorld, the train-selected harness's credited gains hold up on sealed test sets (+9 to +15.5 pp, retaining 86-147% of the training-time gain), and the same paired test flags/rejects harmful candidates.
- Why it matters: This queue already has several self-evolving-harness candidates (Self-Harness, GenericAgent, Harness Updating Is Not Harness Benefit) — GSME is distinct because its entire contribution is an anti-overfitting validation mechanism for exactly the failure mode this repo's own scoreboard keeps finding (a harness edit that looks good on the tasks it was tuned on but doesn't generalize). Worth testing whether the pathology-keyed archive genuinely catches overfit edits or just adds bookkeeping overhead.
- Testability: Feasible small-scale, API only. Build a toy multi-domain suite (a few toy coding/tool tasks standing in for the 3 real benchmarks), implement a simplified pathology-keyed archive (a handful of cells) with Haiku 4.5 generating edits and Sonnet 4.6 as gate/judge, and split train vs. sealed-test tasks to check whether credited gains hold out-of-sample. No GPU. Rough cost: $15-20 — fits the budget; won't match the 7-domain scale, only the directional "does the anti-overfitting gate actually work" check.
- Source: arXiv cs.AI/cs.CL (2607.13683), submitted 2026-07-15

### [Learning to Control LLM Agent Harnesses with Offline Reinforcement Learning](https://arxiv.org/abs/2607.05458)
- Status: proposed — awaiting review
- Claim: Formalizes harness operation as a finite-horizon "Harness MDP" where a lightweight controller selects structural execution actions while the LLM executor stays frozen; the controller is trained purely from offline rollouts via advantage-weighted regression with only terminal task-rubric rewards, plus a post-hoc "Harness Maturity Score" (does the harness follow reliable execution patterns, independent of final-answer correctness). Across 6 controlled domains and 2 public-benchmark adapters (adapted τ-bench retail, adapted AgentBench DB-Bench), the learned controller consistently improves verification behavior and selectively improves final task quality.
- Why it matters: Structurally different from every other self-evolving/self-editing harness candidate already queued — instead of an LLM rewriting prompts or harness code in natural language, a small separately-trained controller picks structural actions while the model itself never changes. Tests whether "learn a control policy over the harness" beats the hand-authored or LLM-authored harness edits this repo has tested and queued so far.
- Testability: Feasible without a GPU strictly required. The controller can be a small classifier/MLP (not an LLM) trained on offline rollout data collected from a toy tool-use task run with Haiku 4.5/Sonnet 4.6 as the frozen executor; compare against a naive baseline and this repo's own already-tested structured harness. Rough cost: $10-20 in API calls for rollout generation; controller training itself is CPU-cheap.
...
- Status: proposed — awaiting review
- Claim: A local-first, persistent, incrementally-updated code knowledge graph (SQLite + Tree-sitter parsing) exposed via 30 MCP tools, claiming 8.2x average context-token reduction for AI coding tools on code-review and large-repo workflows vs. naive full-file reads, while staying within ~1% of real token counts on a typical change set.
- Why it matters: A different context-efficiency mechanism than the conversation-pruning candidates already queued (Less Context Better Agents, TokenPilot) — this pre-builds a structural graph of the codebase itself and serves compact context on demand via MCP tools, rather than pruning conversation/tool-output history. Directly testable in this repo's own MCP lane, potentially against this repo's own codebase.
- Testability: Very feasible on Apple Silicon — fully local (SQLite, Tree-sitter), zero telemetry, only the coding agent's own LLM calls hit an API. Point it at a real small-to-medium repo, compare token usage and review-task quality with vs. without the MCP tools enabled, using Haiku 4.5/Sonnet 4.6 as the coding agent. No GPU. Rough cost: $5-10 in API calls; the 8.2x figure is the vendor's own benchmark, so this would be an independent directional check, not an exact replication.
- Source: GitHub trending (python; topics: mcp, agent-framework)
