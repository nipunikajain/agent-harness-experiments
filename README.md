...
| TokenPilot (cache-efficient context mgmt), Haiku 4.5, 4-hop referral-chain agent task, 3 arms, n=3 | 2026-07-08 | vs no-cache Vanilla: **−69.4%** (in paper's −56–87% band); but −57.4pts is just enabling prompt caching. TokenPilot's own net contribution over a cache-using baseline: **−28.1%** ($0.0650→0.0468/run, disjoint). Accuracy 1.0 across all arms; stress test: harsher reduction never breaks accuracy (recover() safety net) but reverses the cost win past a sweet spot. $1.64 spent | Replicates against the paper's own baseline, but ~⅘ of the headline is prompt caching, not context management; net context-mgmt win ~28%. Reverses to *more* expensive if the prefix falls below the provider cache threshold | [notes](experiments/2026-07-tokenpilot/notes.md) |
